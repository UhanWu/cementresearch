...
```{r}
setwd("~/Desktop/Rdata-science/cementresearch")
limestonedata <- read.csv("data/limestone.csv") |>
  select(Year, limestone) |>
  mutate(limestone = as.double(str_replace(limestone, ",", "")))

limemerge <- bind_cols(limestonedata, limedata) |>
  select(year, limestone, lime)

limestone <- limemerge |>
  mutate(limestone = limestone_interp) |>
  select(year, limestone)
...
combinedata <- bind_cols(c(datacpi, dataunemploy, datagdp, dataoil, 
                           ...
                           limedata, silicadata, limestone)) |>
  mutate(year = `year...1`) |>
  select(year, cpi, unemploy, gdp, oilprice, 
         population, Cement, housing, gasppi,
       workers, lime, silica, limestone)
setwd("~/Desktop/Rdata-science/cementresearch")
write_csv(combinedata, "data/combinedata4.csv")
```
...
stepwise_model <- step(lm(Cement ~ ., data = data_filtered))
...
marginal_effects <- margins(stepwise_model, variables = c("unemploy", "gdp", "housing", 
                                                          "oilprice", "gasppi", "lime"))
...
standardized_model <- lm(formula = scale(Cement) ~ scale(unemploy) + scale(gdp) + 
                           scale(housing) + scale(workers) + scale(lime),
    data = data_filtered)
...
marginal_effects1 <- margins(standardized_model, variables = c("unemploy", "gdp", "housing",
                                                          "workers", "lime"))

plot(marginal_effects1)
